...
/**
 * ResponseEntity:可以以作为控制器方法的返回值，表示响应到浏览器的完整的响应报文
 *
 * 文件上传的要求：
 * 1、form表单的请求方式必须为post
 * 2、form表单必须设置属性为enctype="multipart/form-data"
 *
 *
 */
@Controller
public class FileUpAndDown {
    ...
    @RequestMapping("/test/download")//如果控制器方法没有设置返回值，会将该逻辑地址作为返回值渲染视图
